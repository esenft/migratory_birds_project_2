...
# Try this first. Later you can switch to "present_ge_5" for comparison.
...
# Keep obs_count in the dataframe so we can build lagged features,
# but do NOT use same-week obs_count directly as a predictor.
query = f"""
    SELECT
        species,
        stateProvince,
...
if df.empty:
    raise ValueError(
        "Panel dataframe is empty. Check build_state_week_panel.py output first."
    )
...
# Build lagged / rolling features from PRIOR weeks only
...
# IMPORTANT:
# We do NOT include current obs_count as a feature.
feature_cols = [
    "species",
    "stateProvince",
    "week_of_year",
...
# Save artifacts
model_path = MODEL_DIR / f"{TARGET_COL}_logreg.joblib"
