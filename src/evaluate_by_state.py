...
MODEL_PATH = "models/present_rel_10pct_peak_logreg.joblib"
...
if df.empty:
    raise ValueError("Panel dataframe is empty.")
...
for state, group in test_df.groupby("stateProvince"):
    y_true_st = group["y_true"]
    y_pred_st = group["y_pred"]
    y_proba_st = group["y_proba"]

    try:
        auc = roc_auc_score(y_true_st, y_proba_st)
    except ValueError:
        auc = np.nan

    tn, fp, fn, tp = confusion_matrix(y_true_st, y_pred_st, labels=[0, 1]).ravel()

    rows.append({
        "stateProvince": state,
        "n_rows": len(group),
        "positive_rate": y_true_st.mean(),
        "accuracy": accuracy_score(y_true_st, y_pred_st),
        "precision": precision_score(y_true_st, y_pred_st, zero_division=0),
        "recall": recall_score(y_true_st, y_pred_st, zero_division=0),
        "f1": f1_score(y_true_st, y_pred_st, zero_division=0),
        "roc_auc": auc,
        "tn": tn,
        "fp": fp,
        "fn": fn,
        "tp": tp,
        "is_new_england": state in NEW_ENGLAND_STATES,
    })

results = pd.DataFrame(rows).sort_values("f1", ascending=False)

print("=== TEST METRICS BY STATE ===")
print(results.to_string(index=False, float_format=lambda x: f"{x:.4f}"))
print()

print("=== NEW ENGLAND ONLY ===")
new_england_results = results[results["is_new_england"]].sort_values("f1", ascending=False)
print(new_england_results.to_string(index=False, float_format=lambda x: f"{x:.4f}"))
print()

print("=== HIGHEST ERROR STATES ===")
error_summary = (
    results[["stateProvince", "fp", "fn"]]
